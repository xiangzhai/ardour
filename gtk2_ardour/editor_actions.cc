...
{
	Temporal::MeterPoint const & p = _session->tempo_map().metric_at (ARDOUR_UI::instance()->primary_clock->absolute_time()).meter();
...
{
	Temporal::TempoPoint const & p = _session->tempo_map().metric_at (ARDOUR_UI::instance()->primary_clock->absolute_time()).tempo();
...
Editor::register_region_actions ()
{
	...
	register_toggle_region_action (_region_actions, RegionActionTarget (SelectedRegions|EnteredRegions), "toggle-region-lock-style", _("Glue to Bars and Beats"), sigc::mem_fun (*this, &Editor::toggle_region_time_domain));
