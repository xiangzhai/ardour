...
TempoCurve::TempoCurve (PublicEditor& ed, ArdourCanvas::Container& parent, guint32 rgba, Temporal::TempoPoint const & p, samplepos_t sample, bool handle_events)
	...
	, _shown (false)
	, _color (rgba)
	, _min_tempo (p.note_types_per_minute())
	, _max_tempo (p.note_types_per_minute())
	, _point (p)
	, _start_text (0)
	, _end_text (0)
{
...
#ifdef CANVAS_DEBUG
	group->name = string_compose ("TempoCurve::group for %1", _point.note_types_per_minute());
#endif

	_curve = new ArdourCanvas::FramedCurve (group);
#ifdef CANVAS_DEBUG
	_curve->name = string_compose ("TempoCurve::curve for %1", _point.note_types_per_minute());
#endif
	...
	char buf[10];
	snprintf (buf, sizeof (buf), "%.3f/%d", _point.note_types_per_minute(), _point.note_type());
	_start_text->set (buf);
	snprintf (buf, sizeof (buf), "%.3f", _point.end_note_types_per_minute());
	_end_text->set (buf);
